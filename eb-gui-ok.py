...
class EBibGUI:
    ...
    def setup_dark_theme(self):
        ...
        # Checkbutton Styles - sehr dunkel
...
• german → Findet alle Dateien in deutschen Ordnern"""
    ...
    def line_matches_query(self, line, query_expr):
        """Prüft ob eine Zeile der Query entspricht - mit allen Filtern"""
        if len(line) < 5:
            return False

        # 1. TEXT-SUCHE: Alle Suchbegriffe müssen vorkommen
        if query_expr.strip():
        ...
        # 2. DATEITYP-FILTER prüfen (nur auf Tab 1)
        current_tab = self.get_current_tab()
        if current_tab == "Einfache Suche":
            if not self.check_filetype_filter(line):
                return False

        # 3. JAHRES-FILTER prüfen (nur auf Tab 2)
        if current_tab == "Erweiterte Suche":
            if not self.check_year_filter(line):
                return False
    ...
    def check_filetype_filter(self, line):
        """Prüft Dateityp-Filter"""
        if self.type_vars['all'].get():
            return True

        ext = line[4].lower() if len(line) > 4 else ""

        # Prüfe welche Filter aktiv sind
        if self.type_vars['text'].get() and ext in TAG_DEFS["#text"]:
            return True
        if self.type_vars['audio'].get() and ext in TAG_DEFS["#audio"]:
            return True
        if self.type_vars['image'].get() and ext in TAG_DEFS["#image"]:
            return True

        # Wenn Filter aktiv sind, aber Extension nicht passt
        return False

    def check_year_filter(self, line):
        """Prüft Jahres-Filter basierend auf Spalte 0 (yyyy-mm-dd)"""
        year_from = self.year_from_var.get().strip()
        year_to = self.year_to_var.get().strip()

        # Wenn keine Jahres-Filter gesetzt, alles durchlassen
        if not year_from and not year_to:
            return True

        # Jahr aus Datum extrahieren (Spalte 0: yyyy-mm-dd)
        datum_str = line[0] if len(line) > 0 else ""
        if not datum_str or len(datum_str) < 4:
            return False

        try:
            file_year = int(datum_str[:4])
        except ValueError:
            return False
    ...
    def build_query_from_gui(self):
        """Erstellt Query-String aus GUI-Eingaben"""
        current_tab = self.root.nametowidget(self.root.winfo_children()[0].winfo_children()[0]).select()
        tab_text = self.root.nametowidget(self.root.winfo_children()[0].winfo_children()[0]).tab(current_tab, "text")

        if tab_text == "Einfache Suche":
    ...
    def perform_search(self, query):
        ...
        try:
            ...
            self.root.after(0, lambda q=query: self.results_text.insert(tk.END, f"📊 Verarbeite Daten...\n"))

            # Debug: Filter-Status anzeigen
            current_tab = self.get_current_tab()
            self.root.after(0, lambda: self.results_text.insert(tk.END, f"🔍 Aktueller Tab: {current_tab}\n"))

            if current_tab == "Einfache Suche":
                filter_info = f"📁 Dateityp-Filter: "
                if self.type_vars['all'].get():
                    filter_info += "Alle"
                else:
                    active_filters = []
                    if self.type_vars['text'].get(): active_filters.append("Text")
                    if self.type_vars['audio'].get(): active_filters.append("Audio")
                    if self.type_vars['image'].get(): active_filters.append("Bilder")
                    filter_info += ", ".join(active_filters) if active_filters else "Keine"
                self.root.after(0, lambda info=filter_info: self.results_text.insert(tk.END, f"{info}\n"))

            if current_tab == "Erweiterte Suche":
                year_from = self.year_from_var.get().strip()
                year_to = self.year_to_var.get().strip()
                if year_from or year_to:
                    year_info = f"📅 Jahres-Filter: {year_from or 'alle'} bis {year_to or 'alle'}"
                    self.root.after(0, lambda info=year_info: self.results_text.insert(tk.END, f"{info}\n"))
            ...
            self.root.after(0, lambda: self.results_text.insert(tk.END, f"📁 Lese Datei: {INPUT_FILE}\n"))
                        ...
                        if len(row) >= 4 and self.line_matches_query(row, query):
                            ...
                            if len(found_rows) <= 5:
                                ...
                                self.root.after(0, lambda r=row, d=date_str:
                                               self.results_text.insert(tk.END, f"✓ {d} - {r[3]} ({r[4]})\n"))
